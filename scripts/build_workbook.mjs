...
const names=['统计汇总','事件主表','来源证据','动机与背景标签','省年背景','敏感性','季度统计','描述性关联','待核实与排除','检索覆盖','字段字典','使用说明'];
...
function table(name,rows,fields,labels={},start=1){
 ...
 s.getRange(`A${start}:${last}${start}`).format.fill='#245D72';s.getRange(`A${start}:${last}${start}`).format.font.color='#FFFFFF';s.getRange(`A${start}:${last}${start}`).format.font.bold=true;s.getRange(`A${start}:${last}${start}`).format.rowHeight=32;
 ...
 fields.forEach((f,i)=>{
   ...
   if(['title','classification_reason','key_facts','notes','casualty_note','motive_summary','query','definition','说明'].includes(f)){range.format.columnWidth=55;range.format.wrapText=true;}
   if(['url','field_sources','source_ids','scope_years','scope_regions','supports'].includes(f)){range.format.columnWidth=f==='url'?64:30;range.format.wrapText=true;}
...
sf.getRange('A7:K7').values=[heads];sf.getRange('A7:K7').format.fill='#245D72';sf.getRange('A7:K7').format.font.color='#FFFFFF';sf.getRange('A7:K7').format.font.bold=true;sf.getRange('A7:K7').format.rowHeight=32;
...
table('来源证据',sources,['id','title','publisher','published_date','url','type','independence_group','supports','key_facts','accessed_date'],{id:'来源编号',title:'来源标题',publisher:'发布机构',published_date:'发布日期',url:'原文链接',type:'来源类型',independence_group:'同源组',supports:'支持事件/用途',key_facts:'核读事实与限制',accessed_date:'查阅日期'});
...
table('使用说明',[
 ...
 {主题:'解释',说明:'记录数和密度均受报道、可访问性、检索深度影响。相关不等于因果，地区关系不能推断个人动机。2026核心未检出不表示实际没有随机袭击。'},
